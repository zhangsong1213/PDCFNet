...
class GetTrainingPairs(Dataset):
    ...
    def __init__(self, root, dataset_name, transforms_=None):
        ...
        self.filesA, self.filesB, self.filesC = self.get_file_paths(root, dataset_name)
    ...
    def __getitem__(self, index):
        ...
        img_C = Image.open(self.filesC[index % self.len])
        ...
        img_C = self.transform(img_C) ## C是预处理的图像
    ...
    def get_file_paths(self, root, dataset_name):
        if dataset_name=='EUVP':
            ...
            sub_dirs = ['underwater_imagenet', 'underwater_dark', 'underwater_scenes']  ## 在这里可以设置文件夹
            for sd in sub_dirs:
                filesA += sorted(glob.glob(os.path.join(root, sd, 'trainA') + "/*.*"))
                filesB += sorted(glob.glob(os.path.join(root, sd, 'trainB') + "/*.*"))
                filesC += sorted(glob.glob(os.path.join(root, sd, 'trainC') + "/*.*"))
